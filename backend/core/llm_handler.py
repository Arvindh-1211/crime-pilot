...
FRAUD_EDUCATION: Dict[str, Dict[str, str]] = {
    ...
    "PHISHING": {
        "label": "Phishing (Fake Website / Link Fraud)",
        "what_it_is": (
            "Phishing attacks use fake SMS, emails, or WhatsApp messages to lure you into clicking a link "
            "that opens a fake website — cloned to look exactly like your bank, IRCTC, or government portals. "
            "When you enter your credentials there, fraudsters capture them and drain your accounts."
        ),
        "how_to_protect": (
            "Always check the URL carefully before entering any credentials — the padlock icon does not "
            "guarantee safety. Never click on links in unsolicited messages. Your bank will never send a "
            "link asking you to re-enter your login details."
        ),
        "legal_section": "Section 66C, 66D of IT Act, 2000 — Identity Theft and Online Cheating.",
    },
    "INVESTMENT_SCAM": {
        "label": "Investment / Trading Scam",
        "what_it_is": (
            "Investment scams lure victims into fake stock, cryptocurrency, or forex trading platforms "
            "through WhatsApp or Telegram groups. You are shown fabricated profit screenshots and given "
            "small early 'returns' to build trust. When you invest a large amount, the platform either "
            "locks your funds, demands taxes/fees to release them, or disappears entirely."
        ),
        "how_to_protect": (
            "SEBI-registered brokers do not operate through WhatsApp groups. If someone promises "
            "guaranteed returns of 20-30% per month, it is always a scam. Verify any platform on "
            "SEBI's official website before investing."
        ),
        "legal_section": "Section 420 IPC, SEBI Act 1992 — Fraudulent Investment Schemes.",
    },
    "SEXTORTION": {
        "label": "Sextortion / Sexual Blackmail",
        "what_it_is": (
            "Sextortion involves fraudsters initiating video calls — often from attractive profile photos "
            "on social media — and recording intimate moments. They then threaten to share this recording "
            "with your contacts or on social media unless you pay money. Paying once leads to repeated "
            "demands — it never stops."
        ),
        "how_to_protect": (
            "Do NOT pay any amount — even once. Payment confirms you are vulnerable and demands will escalate. "
            "Block the number immediately. Report the profile on the social platform. File a complaint at "
            "cybercrime.gov.in. You are the victim — there is no shame in reporting this."
        ),
        "legal_section": "Section 67, 67A of IT Act and Section 384 IPC — Extortion and Publishing Obscene Material.",
    },
}
...
class LLMHandler:
    ...
    def assess_description(
        ...
        categories_str = "\n".join(f"- {c}" for c in FRAUD_CATEGORIES)
...
Return JSON only."""
    ...
    def classify_with_llm(self, description: str) -> Optional[str]:
        ...
        categories_str = "\n".join(f"- {c}" for c in FRAUD_CATEGORIES)
        prompt = f"""Classify this cybercrime complaint into exactly ONE category.
...
Return JSON array only."""
    ...
    def get_scenario_questions(self, category_id: str) -> List[str]:
        ...
        return SCENARIO_QUESTIONS.get(category_id, [])

    def generate_scenario_intro(self, category_id: str, category_label: str) -> str:
        """Generate an intro message before scenario questions begin."""
        if not self._initialized:
            return (
                f"I've identified this as a potential case of **{category_label}**. "
                "I'll now ask you a few quick yes/no questions to understand exactly what happened. "
                "Please answer honestly — this helps us build a strong complaint."
            )
        try:
            model = self._make_model(
                "You are a compassionate NCRP cybercrime assistant. Write 2-3 warm sentences. English only."
            )
            response = model.generate_content(
                f"The user's complaint has been identified as '{category_label}'. "
                "Tell them you're going to ask a few scenario-based yes/no questions to understand "
                "exactly what happened to them. Be warm and reassuring — they may be distressed."
            )
            return response.text.strip()
        except Exception:
            return (
                f"I've identified this as a potential case of **{category_label}**. "
                "I'll ask you a few specific yes/no questions now. "
                "Your answers will help us build a complete and accurate complaint."
            )

    def generate_secondary_fraud_intro(self, secondary_category: str) -> str:
        """Generate a message informing user that another fraud type was detected."""
        label = {
            "UPI_FRAUD": "UPI Payment Fraud",
            "VISHING": "Voice Call Impersonation Fraud",
            "PHISHING": "Phishing",
            "INVESTMENT_SCAM": "Investment Scam",
            "SEXTORTION": "Sextortion",
        }.get(secondary_category, secondary_category)

        return (
            f"Based on everything you've told me, I also want to bring something to your attention. "
            f"It appears that you may have also been a victim of **{label}** — which is a separate "
            f"cybercrime that can be filed as an additional complaint. "
            f"Would you like to add this to your report as well? (Yes / No)"
        )

    # ------------------------------------------------------------------
    # NEW: Educational summary generation
    # ------------------------------------------------------------------
    ...
    def generate_educational_summary(self, category_id: str) -> str:
        ...
        return (
            ...
            f"Your complaint helps law enforcement identify and shut down these fraud networks."
        )
